...
class TTT {
  ...
  static placeMove() {
    ...
    if (Screen.grid[row][col] === ' ') {
      ...
      if (ComputerPlayer.getValidMoves(Screen.grid).length > 0) {
        let cpuMove = ComputerPlayer.getSmartMove(Screen.grid, 'X');
        Screen.setMessage(cpuMove);
        Screen.render();
